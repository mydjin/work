...
var auth = {
    check: function (event, checkUrl) {
        ...
        if (event.URL != "" && event.URL != null) {
            ...
            backUrl = event.referrer.replace('http://', '');
            backUrl = backUrl.replace('https://', '');
            backUrl = backUrl.substr(backUrl.indexOf('/'));

        }
        if (checkUrl != "" && checkUrl != null) {
            url = checkUrl.replace('http://', '');
            url = url.replace('https://', '');
            url = url.substr(url.indexOf('/'));
        }

        // debugger;
        var urlAuth = store.get('wechat-admin-user-url-auth');
        var check = 0;
        if (url != null && url != "") {
        ...
        if (check == 0) {
            swal({
                ...
                function () {
                    if (backUrl == null || backUrl == "" || backUrl == "../../sys" || backUrl == '/admin' || backUrl == '/') {
