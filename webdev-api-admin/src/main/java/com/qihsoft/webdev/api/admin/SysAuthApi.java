...
import com.qihsoft.webdev.doc.*;
import com.qihsoft.webdev.core.service.impl.SysMenuAuthService;
import com.qihsoft.webdev.core.service.impl.SysMenuService;
/*import com.qihsoft.webdev.webdev.doc.*;*/
...
public class SysAuthApi extends BaseApi {
    ...
    public Object deleteSubMenu(@PathVariable("id") String id, @RequestHeader("token") String token) {
        ...
        if (!menuService.existId(id))
            return R.error("Id数据异常");
        if (menuService.deleteSubMenu(id, userid)) {
            return R.ok("删除成功");
        } else {
            return R.error("删除失败");
        }
