...
public interface LinkUserMapper extends BaseMapper<LinkUser> {
    ...
    @Select("SELECT a.* " + 
              "FROM v_link_user a " +
            "JOIN (SELECT id from v_link_user where isDel = 0  ${where} " +
            "LIMIT #{index}, #{size})b ON a.id=b.id order by a.createTime asc ") 
    List<JSONObject> getPage(@Param("where") String where, 
                             @Param("index") int index, 
                             @Param("size") int size); 

    @Select("select count(1) total from v_link_user where isDel = 0 ${where} ")
    JSONObject getPageCount(@Param("where") String where);

    @Select("SELECT a.* FROM v_link_user a where 1=1 and isDel=0 ${where}  order by createTime desc")
