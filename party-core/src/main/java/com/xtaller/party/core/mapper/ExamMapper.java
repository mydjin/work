...
public interface ExamMapper extends BaseMapper<Exam> {
    ...
    @Select("SELECT a.*,FROM_UNIXTIME(a.createTime) createTimeStr,FROM_UNIXTIME(startTime) startTimeStr,FROM_UNIXTIME(endTime) endTimeStr FROM exam a where 1=1 and isDel=0 ${where}  order by a.releaseStatus,a.startTime desc")
    ...
    @Update("update exam_candidate set status = 1 , score = null ,reviseTime=0,ipAddr = null,remark = null   where examId = ${examId} and number = ${number}")
