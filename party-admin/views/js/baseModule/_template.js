...
var config = {
    form: '../../form/_template_manage.html',
    form_read: '../../form/_template_read.html',
...
var template = {
    get: function () {
        var param = {url: baseModule.templateManageApi + '/' + index + '-' + size + '-' + key};
    ...
    read: function (event) {
        if (auth.refuse(event))
            return false;
        templateId = getId(event);
        openLay({url: config.form_read, fun: "", title: '模板详细信息'});
        $("#opt-dialog-enter").hide();
        var model = result.get(pageData, templateId);
...
var opt = {
    create: function () {
        ...
        //以下为将数据提交到接口的操作
        var param = {url: baseModule.templateManageApi, data: data};
    ...
    delete: function () {
        var request = ajax.delete({url: baseModule.templateManageApi + '/' + optId});
        request.done(function (d) {
            tips.ok(d.message);
