var index = 1, size = 6, key = '2', totalPage = 0, totalCount = 0, topic = " ", type = "2";
...
var schedule = {
    ...
    delete: function (event) {
        ...
        tips.confirm({message: '是否要删除会议数据？', fun: "opt.delete();"});
    },
    ...
    release: function (event) {
        if (auth.refuse(event))
            return false;
        optId = getId(event);//获取当前id的值
        tips.confirm({message: '是否要发布该会议？', fun: "opt.release();", enter: "发布"});
    ...
    modify: function (event) {
        ...
        optId = getId(event);
        // openLay({url:config.form,fun:"opt.update();",title:config.title});//打开的窗口
        openLay({url: config.modify, fun: 'opt.modify();', title: config.title, enter: "保存"});
        var model = result.get(pageData, optId);
