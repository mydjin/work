package com.xtaller.party.utils.tool;
...
public class WeixinPushUtil {
     ...
     * @param url    跳转链接
     * @param type   模板类型
     * @param data   推送数据包：格式 {"name":{"value":"李文"},"result":{"value":"您已成功注册智慧党建服务平台"}}
     * @return boolean 所代表远程资源的响应结果
     */
    ...
    public static void main(String[] args) {
        sendPush("owVgRxIk3yTZBmWo7e9aqcw-VZbw", "李文", "您已成功注册智慧党建服务平台", "http://mengwp.gxun.edu.cn/partyweb/", "info");
        JSONObject data = new JSONObject();//推送数据包生成
        JSONObject titleObj = new JSONObject();
        titleObj.put("value", "老王");
        data.put("title", titleObj);
        JSONObject contentObj = new JSONObject();
        contentObj.put("value", "您已成功注册智慧党建服务平台");
        data.put("content", contentObj);
