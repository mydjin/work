...
import com.xtaller.party.doc.AcademyInfoCreate;
import com.xtaller.party.doc.AcademyInfoUpdate;
...
public class AcademyInfoApi extends BaseApi {
    ...
    @ApiOperation(value = "读取学院部门信息分页列表")
    ...
    //通过学院部门代码找所在校区 /*需单独配置权限*/
    @GetMapping("/academyInfo/campus/{code}")
    @ApiOperation(value = "通过学院部门代码找所在校区")
    ...
    @GetMapping("/academyInfo/getCatalogue")
    @ApiOperation(value = "读取学院部门支部")
                ...
                for (int rIndex = firstRowIndex; rIndex <= lastRowIndex; rIndex++) {   //遍历行
                    ...
                    if (row != null) {
                        ...
                        if (row.getCell(0) != null && row.getCell(1) != null && row.getCell(2) != null) {
                            JSONObject academyobj = new JSONObject();
                            code = row.getCell(0).toString().trim();//学院部门代码
                            name = row.getCell(1).toString().trim();//学院部门名称
                            ...
                            if (nameexist || codeexist) {
                                academyFlag = false;
                                if (nameexist) {
                                    academyobj.put("info", "学院部门名称已存在");
                                }
                                if (codeexist) {
                                    academyobj.put("info", "学院部门代码已存在");
                                }
                            } else {
                                if (campus.equals("东校区") || campus.equals("西校区") || campus.equals("武鸣校区")) {
                                    ...
                                    if (academyInfo == null) {
                                        academyFlag = false;
                                        academyobj.put("info", "学院部门信息保存失败");
                                    } else {
                                        academyFlag = true;
                            ...
                            if (!academyFlag) {
                                if (V.isEmpty(academyobj.getString("info"))) {
                                    academyobj.put("info", "学院部门信息创建失败");
                                }
